        ...
        <div class="col-md-9 mb-2">
            <div class="card pt-2 view-card-background border-0">
                <div class="card-body">
                    ...
                    <h5 class="card-title text-center"><a class="ai-software-view-card-head" href="{{route('ai_software.view', $ai_tool->slug)}}">{{$ai_tool->name}}</a></h5>
                    <form action="{{route('ai_software.software.like', $ai_tool->id)}}" method="post">
                        @csrf
                        <p class="card-title text-center">
                            <button class="btn btn-info btn-sm" type="submit"><i class="fa fa-heart"></i> Like</button>
                            <a class="btn btn-info btn-sm" href="{{route('ai_software.reviews', $ai_tool->slug)}}"><i class="fa fa-user"></i> {{$reviews->count()}} Reviews</a>
                        </p>
                    </form>
